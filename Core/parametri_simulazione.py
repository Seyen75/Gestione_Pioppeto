...
class ParametriSimulazione:
    def __init__(self):
        ...
        # BILANCIO E PRODUZIONE REALE REALIZZATA
        ...
        # CRONOMETRO DI SISTEMA (ANNO BIOLOGICO E STAGIONI)
    ...
    def avanza_stagione(self) -> dict:
        ...
        # Controlla dinamicamente se l'intero ciclo aziendale si è concluso
        ...
        # Verifica dinamica della conclusione del ciclo: la simulazione finisce quando non ci sono più lotti attivi
        fine_simulazione = self.verifica_tutti_lotti_conclusi()

        return {
            "stagione": self.stagione_corrente,
            "anno": self.anno_corrente,
            "cambio_anno_rilevato": cambio_anno,
            "simulazione_terminata": fine_simulazione
        }

    def verifica_tutti_lotti_conclusi(self) -> bool:
        # Controlla lo stato di tutti i lotti in campo.
        # Se la collezione è vuota o se ogni singolo lotto è stato tagliato, restituisce True.
        
        if not self.collezione_lotti:
            return True

        # Ritorna True solo se l'espressione è vera per ogni lotto della lista
        return all(lotto.tagliato for lotto in self.collezione_lotti)
    ...
    def reset_simulazione_globale(self):
        ...
        # Resetta biologicamente ogni lotto presente in memoria
        for lotto in self.collezione_lotti:
            lotto.inizializza_nuovo_ciclo()

    def registra_istantanea_stato_corrente(self, dati_quadro: dict):
